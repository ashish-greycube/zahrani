...
value_fields = ("income","cost_goods","expense", "gross_profit_loss")
	...
	for d in accounts:
		...
		for entry in gl_entries_by_account.get(d.name, []):
			if cstr(entry.is_opening) != "Yes":
				...
				d["gross_profit_loss"] = d.get("income") - d.get("expense") - d.get("cost_goods")

		total_row["income"] += d["income"]
		total_row["expense"] += d["expense"]
		total_row["cost_goods"] += d["cost_goods"]

	total_row["gross_profit_loss"] = total_row.get("income") - total_row.get("expense")- total_row.get("cost_goods")
...
def get_columns(filters):
	return [
		...
		{
			"fieldname": "expense",
			"label": _("Expense w/o Cost of Goods"),
			"fieldtype": "Currency",
			"options": "currency",
			"width": 170
		},
